...
const defaultProps = {
  text: '',
  category: 'home',
  startTime: `${formatDate(new Date(),'HH')}:${getMinutes(new Date())}`,
  endTime: `${formatDate(addHours(new Date(), 1), 'HH')}:${getMinutes(new Date())}`
}
...
class CalendarReminderForm extends React.Component {
  ...
  handleStartTimeChange = e => {
	if(e.target.value < formatDate(new Date(), 'HH:mm')){
      return;
    }
